...
#include "Vtt_um_algofoogle_vga.h" // Generated by Verilator

#define DESIGN      tt_um_algofoogle_vga
#define VDESIGN     Vtt_um_algofoogle_vga
#define MAIN_TB     Vtt_um_algofoogle_vga_TB
...
void get_text_and_rect(
  ...
  SDL_Color bg = {0, 50, 100};

  surface = TTF_RenderText_Shaded(font, text, textColor, bg);
    ...
    if (SDL_QUIT == e.type) {
      ...
      switch (e.key.keysym.sym) {
        ...
        case SDLK_v:
          ...
          printf("Logging VSYNC %s with frame_counter=%d\n", TB->log_vsync ? "enabled" : "disabled", TB->frame_counter);
          break;
        case SDLK_KP_PLUS:
...
void render_info_text(SDL_Renderer* renderer, TTF_Font *font) {
  // #ifdef INSPECT_INTERNAL
...
// #endif
  ...
  while (!gQuit) {
    ...
    if (TB->paused) {
      ...
      SDL_WaitEvent(NULL); // If we're paused, an event is needed before we could resume.
    }
    ...
    for (int i = 0; i < gRefreshLimit; ++i) {
      ...
      // int pixel_lit = TB->m_core->uo_out & 0b01110111;
      int pixel_lit = TB->r() | TB->g() | TB->b();
      ...
      if (x >= 0 && x < WINDOW_WIDTH && y >= 0 && y < WINDOW_HEIGHT) {
        ///////////////////////////////////////////--- colour bit 7 ---////////////////////////--- colour bit 6 ---//
        int red   = TB->rscaled(); // gRenderDAC ? TB->rscaled() : (((TB->m_core->uo_out & 0b0000'0001) << 7) | ((TB->m_core->uo_out & 0b0001'0000) << 2));
        int green = TB->gscaled(); // gRenderDAC ? TB->gscaled() : (((TB->m_core->uo_out & 0b0000'0010) << 6) | ((TB->m_core->uo_out & 0b0010'0000) << 1));
        int blue  = TB->bscaled(); // gRenderDAC ? TB->bscaled() : (((TB->m_core->uo_out & 0b0000'0100) << 5) | ((TB->m_core->uo_out & 0b0100'0000) << 0));
        if (gDisableR) red = 0;
        if (gDisableG) green = 0;
        if (gDisableB) blue = 0;
        framebuffer[(y*WINDOW_WIDTH + x)*4 + 2] = red   | (TB->hsync_asserted() ? 0b1000'0000 : 0) | speaker;  // R
        framebuffer[(y*WINDOW_WIDTH + x)*4 + 1] = green;                                                    // G
        framebuffer[(y*WINDOW_WIDTH + x)*4 + 0] = blue  | (TB->vsync_asserted() ? 0b1000'0000 : 0) | speaker;  // B.
      }
